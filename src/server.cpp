...
namespace
{
        ...
        void server_client_alloc( uv_handle_t* handle, size_t suggested_size, uv_buf_t* buf )
        {
                auto& c = (server_client&) ( *handle );
        ...
        void server_new_conn( uv_stream_t* srv, int status )
        {
                ...
                uv_tcp_init( s.loop, c.h() );
                if ( uv_accept( srv, (uv_stream_t*) &c ) == 0 ) {
                        spdlog::info( "Accepted new connection" );
                        std::tie( c.ip, c.port ) = get_connection_info( c.h(), sock_kind::PEER );
                        spdlog::info( "Client address {}:{}", c.ip, c.port );
                        s._commit_client( c );
                        uv_read_start( (uv_stream_t*) &c, server_client_alloc, server_client_read );
                } else {
                        spdlog::error( "Accepting new connection error {}", uv_strerror( status ) );
                        uv_close( (uv_handle_t*) &c, []( uv_handle_t* handle ) {
                                auto& c = (server_client&) ( *handle );
...
int server_init( server& s, uv_loop_t* loop, int port )
{
        ...
        uv_tcp_bind( &s, (const struct sockaddr*) &s.addr, 0 );
        spdlog::info( "Starting new server on port {}", port );
        return uv_listen( (uv_stream_t*) &s, server::backlog, trctl::server_new_conn );
}
